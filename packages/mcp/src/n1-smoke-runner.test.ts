...
const STUB_SDK_ADAPTER = {
  ...
  buildMandateSignRequest: async () => {
    return {
      result: {
        // Smoke runner only validates outputSchema + expect field existence;
        // no need to construct full typedData here.
        typedData: {},
        mandate: {
...
test("N1 fixtures smoke runner: run steps.json in order with partial outputSchema checks", async (t) => {
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    // First ensure fixture input itself satisfies the frozen inputSchema
    // (otherwise the fixture is invalid).
    assert.equal(
    ...
    // MCP responses must satisfy the frozen outputSchema (hard contract constraint).
    assert.equal(
    ...
    if (step.expect !== undefined) {
      // fixture expect is a partial shape (same idea as validatePartial in scripts/contract-check.mjs)
      assert.ok(isObject(step.expect), `steps[${i}].expect must be object`);

      const structured = (result.structuredContent ?? {}) as Record<string, unknown>;
      const actualResult = (structured.result ?? {}) as unknown;

      // Current N1 runs in a no-RPC/mock environment: we only check shape
      // (most expect values are placeholders), and avoid deep equality assertions
      // so smoke runner does not become an on-chain truth assertion.
