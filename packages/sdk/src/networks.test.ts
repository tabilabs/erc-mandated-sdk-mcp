...
test("getSupportedChains returns BSC Testnet first and Sepolia as fallback", () => {
...
test("getChainConfig defaults to BSC Testnet", () => {
...
test("getChainConfig selects Sepolia when chainId=11155111", () => {
...
test("getRpcUrl reads from environment variable", () => {
...
test("getRpcUrl throws a structured error when env is missing", () => {
  const previous = process.env.BSC_TESTNET_RPC_URL;
...
test("external mutation of returned supported chains does not pollute internal config", () => {
  const chains = getSupportedChains();
...
test("getChainConfig throws UNSUPPORTED_CHAIN for unknown chainId", () => {
  assert.throws(
    () => getChainConfig(1),
    (error: unknown) => {
      assert.ok(error instanceof NetworkConfigError);
...
test("getRpcUrl throws structured error for Sepolia when env is missing", () => {
